...
export function describeCourse(status: CourseStatus): string {
    switch (status.status) {
        ...
        case "ACTIVE":
            return `Active course with ${status.enrolledCount} students, starting ${status.startDate}`;

        case "ARCHIVED":
            return `Archived course with final enrollment of ${status.finalEnrollmentCount} students`;

        case "CANCELLED":
            return `Cancelled course because: ${status.reason}`;
